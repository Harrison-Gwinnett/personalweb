  ...
  return (
    <FadeUp delay={delay} style={{ borderTopColor: '#D7D5C9', borderTopStyle: 'solid', borderTopWidth: '1px', display: 'flex', gap: '20px', paddingTop: '36px', width: '100%' }}>
      ...
      <div style={{ display: 'flex', flex: 1, flexDirection: 'column', gap: '14px', minWidth: 0 }}>
        ...
        {role.summary && (
          <p style={{ color: '#41443B', fontFamily: GS, fontSize: '16px', letterSpacing: '-0.005em', lineHeight: '27px', margin: 0, maxWidth: '600px' }}>
            {role.summary}
          </p>
        )}

        {role.paragraphs?.map((p) => (
          <p key={p.slice(0, 24)} style={{ color: '#41443B', fontFamily: GS, fontSize: '16px', letterSpacing: '-0.005em', lineHeight: '27px', margin: 0, maxWidth: '600px' }}>
            {p}
          </p>
        ))}

        {role.bullets && role.bullets.length > 0 && (
          <ul style={{ display: 'flex', flexDirection: 'column', gap: '8px', listStyle: 'none', margin: 0, maxWidth: '600px', padding: 0 }}>
            {role.bullets.map((item) => (
              <li key={item} style={{ color: '#41443B', display: 'flex', fontFamily: GS, fontSize: '16px', gap: '10px', letterSpacing: '-0.005em', lineHeight: '27px' }}>
        ...
        {role.closing && (
          <p style={{ color: '#5E6157', fontFamily: GS, fontSize: '15px', fontStyle: 'italic', letterSpacing: '-0.005em', lineHeight: '24px', margin: 0, maxWidth: '600px' }}>
            {role.closing}
          </p>
        )}
    ...
    <div style={{ alignItems: 'center', backgroundColor: '#F3F1E9', display: 'flex', flexDirection: 'column', fontSize: '12px', fontSynthesis: 'none', lineHeight: '16px', minHeight: '100dvh', paddingBottom: '72px', paddingTop: '56px' }}>
      <div style={{ display: 'flex', flex: 1, flexDirection: 'column', width: '768px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '18px', paddingBottom: '48px', paddingTop: '104px', width: '100%' }}>
          ...
          <FadeUp delay={0.18}>
            <p style={{ color: '#41443B', fontFamily: GS, fontSize: '18px', letterSpacing: '-0.005em', lineHeight: '31px', margin: 0, maxWidth: '600px' }}>
